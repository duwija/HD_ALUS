...
$gateway = config('services.whatsapp.gateway_url');

Route::prefix('wa')->group(function () use ($gateway) {
	...
	Route::post('start', function (\Illuminate\Http\Request $request) use ($gateway) {
		$session = $request->input('session');
		$response = Http::post("{$gateway}/api/start", [ "session" => $session ]);
		return response()->json($response->json());
	});

	Route::get('status', fn() => response()->json(Http::get("{$gateway}/api/health")->json()));
	Route::get('{session}/status', fn($session) =>
		response()->json(Http::get("{$gateway}/api/{$session}/qr")->json())
	);
