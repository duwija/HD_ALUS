...
# ==========================================================
# Usage:
# python3 reset_cdata_epon_ont.py <ip> <username> <password> <port> <timeout> <card> <pon> <onu>
# Example:
# python3 reset_cdata_epon_ont.py 103.156.75.196 root public 1612 10 1 1 1
#
...
#   OLT(config-epon-0/<card>)# ont restore-factory <pon> <onu>
...
# in "ont restore-factory <pon> <onu>" is then the port NUMBER WITHIN that card (1-4 on
# units seen so far), and <onu> the ONU id — same two-argument shape as GPON's
# "ont restore-factory <pon> <onu>", just scoped to the already-selected card.
...
def telnet_cdata_epon_reset(host, port, username, pwd, card, pon, onu, log_path):
    ...
    with open(log_file_path, 'a') as log_file:
        try:
            ...
            reset_cmd = f'ont restore-factory {pon} {onu}\r\n'.encode()
            log(f"SEND RESET COMMAND: ont restore-factory {pon} {onu}", log_file)
            s.sendall(reset_cmd)
            out = recv_until(s, '#', wait=15)
            decoded = out.decode('ascii', errors='ignore')
            log(f"RESET COMMAND OUTPUT: {repr(decoded)}", log_file)
            ...
            if any(w in lowered for w in ('error', 'invalid', 'fail', 'not exist', 'unknown command')):
                ...
                print(f"error:Failed to factory-reset ONU EPON 0/{card}/{pon}/{onu} - {cleaned[:150]}")
            else:
                log("RESET COMMAND SENT", log_file)
                print(f"success:ONU EPON 0/{card}/{pon}/{onu} factory-reset command sent!")
...
# ---------- RUN ----------
telnet_cdata_epon_reset(ip, port, login, password, card_int, pon_int, onu_num, log_path)
