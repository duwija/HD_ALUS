...
# ==========================================================
# Usage:
# python3 delete_cdata_ont.py <ip> <username> <password> <port> <timeout> <pon> <onu>
# Deletes (de-registers) a CDATA GPON ONU via Telnet CLI:
#   OLT> enable
#   OLT# config
#   OLT(config)# interface gpon 0/0
#   OLT(config-gpon-0/0)# ont delete <pon> <onu>
...
def telnet_cdata_delete(host, port, username, pwd, pon, onu, log_path):
    ...
    with open(log_file_path, 'a') as log_file:
        try:
            ...
            if any(w in lowered for w in ('error', 'invalid', 'fail', 'not exist', 'unknown command')):
                ...
                print(f"error:Failed to delete ONU PON{pon}/{onu} - {cleaned[:150]}")
            else:
                log("DELETE COMMAND SENT", log_file)
                print(f"success:ONU PON{pon}/{onu} delete command sent!")
...
telnet_cdata_delete(ip, port, login, password, pon_int, onu_num, log_path)
