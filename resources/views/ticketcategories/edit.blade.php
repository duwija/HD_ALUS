        ...
        <div class="col-xl-6 col-lg-7 col-12">
            <div class="tc-card">
                <div class="tc-card-header">
                  <h5><i class="fas fa-edit"></i>Edit Kategori Tiket</h5>
                  <a href="{{ route('ticketcategories.index') }}" class="btn btn-sm btn-secondary" style="border-radius:8px;font-size:12px">
                    <i class="fas fa-arrow-left mr-1"></i>Kembali
                  </a>
                </div>

                <form action="{{ route('ticketcategories.update', $category->id) }}" method="POST" id="editCategoryForm">
                    @csrf
                    @method('PUT')
                    <div class="tc-card-body">
                        ...
                        <div class="form-group">
                            ...
                            <small class="form-text text-muted mb-2">
                                Kelola langkah-langkah workflow untuk kategori ini. Tekan Enter atau klik tombol + untuk menambah step baru.
                            </small>
                            
                            <!-- Current Workflow Steps -->
                            <div id="current-workflow" class="mb-3"></div>

                            <!-- Add New Step -->
                            <div id="workflow-container">
                                <label class="font-weight-bold mb-2">Tambah Step Baru:</label>
                            ...
                            <input type="hidden" name="workflow" id="workflow-hidden">
                        </div>
                    </div><!-- /.tc-card-body -->
...
$(document).ready(function() {
    ...
    // Initialize display dengan workflow existing
    updateWorkflowDisplay();
    updateWorkflowHidden();

    // Add workflow step
    $('.add-workflow-btn').on('click', function() {
        const input = $('#new-workflow-input');
    ...
    // Enter key untuk tambah step
    $('#new-workflow-input').on('keypress', function(e) {
        if (e.which === 13) {
            e.preventDefault();
    ...
    function updateWorkflowDisplay() {
        ...
        if (workflowSteps.length === 0) {
            html = `<div class="wf-empty"><i class="fas fa-info-circle mr-1"></i>Belum ada workflow step. Tambahkan step baru di bawah.</div>`;
            $('#current-workflow').html(html);
        } else {
            html = `
                <div class="wf-box">
                    <div class="wf-box-title">
                        <span><i class="fas fa-list-ol mr-1"></i>Workflow Steps</span>
