...
<div class="container-fluid">
    ...
    <nav aria-label="breadcrumb" class="mb-3">
        <ol class="breadcrumb bg-transparent p-0">
            ...
            <li class="breadcrumb-item"><a href="{{ route('admin.tenants.show', $tenant->id) }}">{{ $tenant->name }}</a></li>
            <li class="breadcrumb-item active">Laravel Log</li>
        </ol>
    </nav>
    ...
    <div class="d-flex align-items-center justify-content-between mb-3">
        <div>
            <h2 class="h5 mb-0">
                ...
                Laravel Log — <strong>{{ $tenant->name }}</strong>
                <span class="badge badge-secondary ml-1">{{ $rescode }}</span>
            </h2>
            @if($fileSize !== null)
            <small class="text-muted">
                <code>storage/logs/tenant_{{ $rescode }}/laravel.log</code>
                &bull; {{ $fileSize > 1048576 ? number_format($fileSize/1048576,2).' MB' : number_format($fileSize/1024,1).' KB' }}
        ...
        <div class="d-flex align-items-center">
            @if($content !== null)
            <form method="GET" action="{{ route('admin.tenants.log', $tenant->id) }}" class="d-inline-flex align-items-center mr-2">
            ...
            @endif
            <a href="{{ route('admin.tenants.show', $tenant->id) }}" class="btn btn-sm btn-outline-secondary">
    ...
    <div class="alert alert-warning">
        ...
        File log belum ada: <code>{{ $path }}</code>
    </div>
    @else
