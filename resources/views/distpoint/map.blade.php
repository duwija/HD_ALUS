...
<style>
  ...
  /* Layer Items — compact single-row */
  .layer-item {
    background: white;
    padding: 5px 7px;
    border-radius: 6px;
  ...
  /* Hide description and date — too noisy */
  .layer-item-info, .layer-item-date { display: none; }
  ...
  <div class="map-layout-container">
    ...
    <div id="layerSidebar">
      ...
      <div class="sidebar-body">
        ...
        <div class="location-search-container mb-1">
        ...
        <div class="d-flex gap-1 mb-1 align-items-center">
          <input id="layerSearchInput" type="text" class="form-control form-control-sm" placeholder="Cari layer..." oninput="filterLayerList()" style="font-size:11px">
          <button class="btn btn-create-group" type="button" onclick="showCreateGroupInput()" title="Buat Group Baru" style="white-space:nowrap;flex-shrink:0">
      ...
      function renderLayerListGrouped() {
        ...
        if (groupNames.length > 0) {
          html += `<div class='no-group-drop-area' ondragover='event.preventDefault()' ondrop='onDropLayerToNoGroup(event)' style='min-height:18px; background:#f8f9fa; border:1px dashed #ccc; border-radius:4px; margin-bottom:5px; text-align:center; color:#bbb; font-size:10px; padding:3px;'>↓ drag ke sini untuk keluar group</div>`;
        }
      ...
      function renderLayerItemHTML(layer) {
        ...
        if (layer.type === 'marker' && layer.icon === 'text') {
          typeIcon = '<i class="fas fa-font"           style="color:#f57c00;font-size:11px;width:13px;text-align:center" title="Text"></i>';
        } else if (layer.type === 'marker') {
          typeIcon = '<i class="fas fa-map-marker-alt" style="color:#0288d1;font-size:11px;width:13px;text-align:center" title="Marker"></i>';
        } else if (layer.type === 'polyline') {
          typeIcon = '<i class="fas fa-route"          style="color:#5c6bc0;font-size:11px;width:13px;text-align:center" title="Polyline"></i>';
        } else if (layer.type === 'polygon') {
          typeIcon = '<i class="fas fa-draw-polygon"   style="color:#388e3c;font-size:11px;width:13px;text-align:center" title="Polygon"></i>';
        } else if (layer.type === 'rectangle') {
          typeIcon = '<i class="fas fa-vector-square"  style="color:#388e3c;font-size:11px;width:13px;text-align:center" title="Rectangle"></i>';
        } else if (layer.type === 'text') {
          typeIcon = '<i class="fas fa-font"           style="color:#f57c00;font-size:11px;width:13px;text-align:center" title="Text"></i>';
        } else {
          typeIcon = '<i class="fas fa-layer-group"    style="color:#888;font-size:11px;width:13px;text-align:center" title="' + (layer.type||'Layer') + '"></i>';
        }
        ...
        return `<div class="layer-item" data-layer-id="${layer.id}" draggable="true" ondragstart="onDragLayer(event, ${layer.id})" ondragover="onLayerDragOver(event)" ondragleave="onLayerDragLeave(event)" ondrop="onLayerDrop(event, ${layer.id})">
          <div class="layer-item-name">
            ...
            <span class="layer-text" title="${name}">${name}</span>
          </div>
          <div class="layer-item-actions">
            ...
            <button class="btn btn-sm btn-danger"  title="Hapus"  onclick="event.stopPropagation();deleteLayer(${layer.id})"><i class="fas fa-trash"></i></button>
          </div>
        </div>`;
      }
