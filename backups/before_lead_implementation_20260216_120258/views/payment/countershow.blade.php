...
<section class="content-header">
  ...
  <div class="card card-primary card-outline">
    <div class="card-header">
      ...
      $merchant_fee = $suminvoice_number->merchant_fee ?? 0;
...
<!-- /.card-header --> <div class="card-body row">
          ...
          <tbody>
            ...
            <tr> <td colspan="5"> <strong> Subtotal</strong></td>
              <td colspan="2">
               ...
               elseif ($status == 1 && !is_null($merchant_fee))
               {
                 $admin_fee  = $merchant_fee;
               }
               else
               {$admin_fee  = 0;}
               ...
               $total = (float)$subtotal + (float)$tax - (float)$pph;
               $totalwithfee = (float)$subtotal + (float)$tax - (float)$pph + (float)$admin_fee;
               @endphp
              ...
              @endif
              @if ($status == 1 && !is_null($merchant_fee))
              <tr>
                <td colspan="5"><strong>Biaya Admin</strong></td>
                <td colspan="2">
              ...
              <tr> <td colspan="5"> <strong> Pph 23</strong></td>
                <td colspan="2">
                 ...
                 <tr> <td colspan="5"> <strong> Total</strong></td>
                  <td colspan="2">
                    <strong id="total"> {{ number_format($totalwithfee, 0, ',', '.') }}</strong> </td></tr>
 ...
 <!--            <button type="button" class="float-left btn p-2 bg-gradient-warning btn-sm btn-warning mr-2 mb-2" data-toggle="modal" data-target="#cancelModal">
            ...
            <a href="{{url('payment').'/' .$invoice->tempcode. '/print'}}" target="_blank" class="btn btn-primary float-left mr-2 ">Print</a>
            <a href="{{url('payment').'/' .$invoice->tempcode. '/dotmatrix'}}" target="_blank" class="btn btn-primary float-left mr-2">Print Thermal</a>
                ...
                <form role="form" method="post" action="/suminvoice/{{$suminvoice_number->id }} ">
                  ...
                  <div class="card-body row">
                    <div class="form-group m-0 col-md-12">
                     ...
                     <strong>{{$customer->customer_id}} | {{$customer->name}} </strong></br>
                     ...
                     <a>{{$suminvoice_number->number}} </a>
                     <hr>
                   </div>
                   

                   <div class="form-group m-0 col-md-6">
                    <label for="invoice_type">  Amount | admin Fee</label>
                    <div class="input-group mb-3">
                      ...
                      <input type="text" class="form-control @error('merchant_fee') is-invalid @enderror" 
                      name="merchant_fee" id="merchant_fee"  
                      placeholder="Rp. XXXX.XXXX" value="{{ $admin_fee }}" readonly>
                      @else
                      <input type="text" class="form-control @error('recieve_payment') is-invalid @enderror" 
                      name="recieve_payment" id="recieve_payment"  
