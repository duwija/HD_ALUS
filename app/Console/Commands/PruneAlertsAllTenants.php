...
class PruneAlertsAllTenants extends Command
{
    protected $signature   = 'alerts:prune-all
                            {--tenant= : Filter by tenant domain (optional)}
                            {--days=7 : Retention in days}
                            {--chunk=5000 : Rows deleted per batch}';
    protected $description = 'Delete alerts records older than N days (default 7) for ALL active tenants';
    ...
    protected function pruneForCurrentTenant(int $days, int $chunk)
    {
        if (!Schema::hasTable('alerts')) {
            $this->line('  No alerts table for this tenant.');
            return;
        }

        // idx_created_at bikin WHERE created_at < ... pakai range scan, bukan full table scan.
        try {
            DB::statement('ALTER TABLE alerts ADD INDEX IF NOT EXISTS idx_created_at (created_at)');
        } catch (\Exception $e) {
            $this->warn('  Gagal memastikan index created_at: ' . $e->getMessage());
        }
